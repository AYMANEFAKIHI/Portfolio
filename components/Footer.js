...
const Footer = () => {
  ...
  const services = [t('svc1'), t('svc2'), t('svc3'), t('svc4'), t('svc5')]
      ...
      <div className="container-max section-padding">
        <motion.div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8"
          ...
          <div className="md:col-span-2">
            ...
            <p className="text-sm mb-4 font-medium tracking-wide" style={{ color: 'var(--accent)' }}>{t('footer_tagline')}</p>
            <p className="mb-6 max-w-md text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>{t('footer_desc')}</p>
            <div className="flex items-center gap-2 mb-6">
              <span className="w-2 h-2 rounded-full bg-[#14b8a6] animate-pulse" />
              <span className="text-sm" style={{ color: 'var(--accent)' }}>{t('footer_available')}</span>
            </div>
            <div className="flex space-x-3">
              {[
                { href: 'https://www.linkedin.com/in/aymane-fakihi-9a3435335/', label: 'LinkedIn', hoverBg: '#0077b5',
                  icon: <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg> },
                { href: 'https://github.com/AYMANEFAKIHI', label: 'GitHub', hoverBg: '#444',
                ...
                { href: 'mailto:faymane12@gmail.com', label: 'Email', hoverBg: '#14b8a6',
                ...
                <motion.a key={social.label} href={social.href}
                  ...
                  whileHover={{ scale: 1.1, background: social.hoverBg, color: '#fff' }}
                  whileTap={{ scale: 0.9 }}
                >
                  {social.icon}
                </motion.a>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {services.map((s, i) => (
                <li key={i} className="text-sm" style={{ color: 'var(--text-muted)' }}>{s}</li>
              ))}
            </ul>
          </div>
        </motion.div>
        ...
        {/* Bottom */}
        <motion.div className="border-t pt-8 flex flex-col md:flex-row justify-between items-center gap-4"
